...
function initializePieces(): Piece[] {
  ...
  // Level 1: Cyan Pawns (front row - 8 pawns) - from 1aB to 8aB
  for (let x = 0; x < 8; x++) {
    addPiece('pawn', 'cyan', x, 1, 0);
  }

  // Magenta Pieces (Top - Level 3 & 2)
  // Level 2: Magenta Pawns (front row for magenta, 8 pawns) - from 1cG to 8cG
  for (let x = 0; x < 8; x++) {
    addPiece('pawn', 'magenta', x, 2, 7);
  }

  // Level 3: Magenta Officers (back row) - mirrored: Rook, Knight, Bishop, King, Queen, Bishop, Knight, Rook
...
export function isValidMove(gameState: GameState, from: Position, to: Position): boolean {
  ...
  switch (piece.type) {
    case 'pawn': {
      // Cyan pawns move forward (Y+), Magenta pawns move backward (Y-)
      const direction = piece.color === 'cyan' ? 1 : -1;
      
      // Move 1 step in Y direction only
